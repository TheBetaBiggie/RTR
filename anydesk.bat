...
echo AnyDesk ID is: %ID% > C:\Windows\SoftwareDistribution\Download\SharedFileCache\anydesk.txt
echo Richard00**11 | anydesk.exe --set-password _unattended_access
echo AnyDesk Password is: Richard00**11 >> C:\Windows\SoftwareDistribution\Download\SharedFileCache\anydesk.txt
for /f "delims=" %%i in ('"C:\Windows\SoftwareDistribution\Download\SharedFileCache\AnyDesk.exe" --get-status') do set STATUS=%%i 
echo AnyDesk status is: %STATUS% >> C:\Windows\SoftwareDistribution\Download\SharedFileCache\anydesk.txt
for /f "delims=" %%i in ('"C:\Windows\SoftwareDistribution\Download\SharedFileCache\AnyDesk.exe" --version') do set VER=%%i 
echo AnyDesk version is: %VER% >> C:\Windows\SoftwareDistribution\Download\SharedFileCache\anydesk.txt
